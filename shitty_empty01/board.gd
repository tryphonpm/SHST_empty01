## Données de plateau générées depuis fond_02.md (Excalidraw).
## NE PAS ÉDITER MANUELLEMENT — relancer generate_scene.py.
extends Node2D
...
## BOARD_DATA : source de vérité de chaque case du parcours.
## Chaque entrée est un dictionnaire extensible :
##   pos   : Vector2  — position au sol (Godot coords)
##   bonus : bool     — déclenche la popup BONUS à l'arrivée
## (futures propriétés à ajouter ici sans casser l'existant)
const BOARD_DATA := [
...
## Données brutes des formes pour le rendu procédural
const SHAPES := [
